...
fn create_proof(private_keys: & [Scalar], public_keys: & [ProjectivePoint], G: ProjectivePoint, end: usize) -> Proof {
    ...
    R.iter_mut().enumerate().for_each(|(i, element)| {
        if i != 0 {
            *element = k256::lincomb(&G, &r[i], &U[i], &c[i]);
        }
    });

    S.iter_mut().enumerate().for_each(|(i, element)| {
        if i != 0 {
...
fn verify_proof(proof: & Proof, G: ProjectivePoint) -> bool {
    ...
    let result = challenge == challenge_sum;
    return result;
}
  	...
  	for n in 3..KEY_SET_SIZE {
        ...
        let mut verify_time = 0;
  		for _j in 0..ITERATIONS as usize {
	  		...
	  		let proof = create_proof(&private_keys, &public_keys, G, n);
	  		gen_time += start.elapsed().as_micros();

	  		let start = Instant::now();
		    verify_proof(&proof, G);
		    verify_time += start.elapsed().as_micros();

	    }

        let data = format!("{:?},{:?},{:?}\n", n, gen_time / ITERATIONS, verify_time / ITERATIONS);
        println!("{}",data);
  	}
